...
def potential(lmp, args):
    ...
    if lmp.variables["thermostat"].value == 1:
        lmp.fix(5, "all", "langevin", lmp.variables["temp"].value, lmp.variables["temp"].value, lmp.variables["tdamp"].value, int(lmp.variables["seed"].value))

    lmp.fix("avp", "all", "ave/time", int(lmp.variables["nevery"].value), int(lmp.variables["nrepeat"].value), int(lmp.variables["nfreq"].value), "c_thermo_press", "mode vector")

    # Setup output
    lmp.thermo(int(lmp.variables["nthermo"].value))
...
def displace(lmp, args, idir):
    ...
    # Run MD
    lmp.run(int(lmp.variables["nequil"].value))
    lmp.run(int(lmp.variables["nrun"].value))
...
def elastic():
    ...
    L.run(int(L.variables["nequil"].value))

    print L.variables["thermostat"].value

    L.run(int(L.variables["nrun"].value))
